  ...
  try {
    ...
    if (email) {
      ...
      if (existingSnapshot.exists()) {
        const existingUsers = existingSnapshot.val();
        const conflictingId = Object.keys(existingUsers).find(id => id !== userId);
        if (conflictingId) {
          return res.status(409).json({ error: 'Email already in use by another account', status: 409 });
        }
      }
      updates.email = email;
    }
